...
4. Verify data persistence: search the `handleAutosave` function body for both `methodology` and `equipment` field names to confirm they are included in the autosave PATCH payload. Run: `grep -n "methodology\|equipment" src/app/(dashboard)/reports/[id]/edit/page.tsx` and confirm both field names appear in (a) the FormData interface, (b) the fetchReport data mapping, (c) the handleAutosave body, and (d) the handleSubmit body. All four locations must reference both fields.
  </verify>
  <done>
...
- Autosave includes both new fields in its PATCH payload (confirmed by grep showing methodology and equipment in handleAutosave body)
- Both fields round-trip correctly: fetchReport maps DB format to form strings, handleSubmit/handleAutosave maps form strings back to DB format
  </done>
</task>
...
5. Both fields present in handleAutosave and handleSubmit bodies (data persistence confirmed)
6. Inspector guide updated with new field documentation
</verification>
...
- Both fields participate in autosave (methodology and equipment present in handleAutosave JSON body)
