...
must_haves:
  truths:
    ...
    - "PDF output includes a visually prominent Section 3.5 Limitations with page-break-before, amber warning box, bold heading, and court disclaimer — unmissable for court admissibility"
...
**Section layout (two pages — main methodology on page 1, limitations on page 2):**

**PAGE 1:** Standard methodology page using the same header/footer pattern as Declaration (Header component with reportNumber, Footer component at bottom).
...
**PAGE 2:** Dedicated limitations page with `page-break-before`. This page MUST be visually prominent and unmissable for court admissibility.

**3.5 Limitations & Restrictions** (subsection rendered as a standalone page)

This section gets its own page via a second `<Page>` element within the component. Use the same header/footer pattern.

At the top of the page, render a full-width amber warning box:
- Background: "#fffbeb" (light amber)
- Border: 3px solid left border using `colors.ranzYellow` (or "#f59e0b" amber-500)
- Padding: 16 all sides
- The box contains:
  - A bold heading inside the box: "IMPORTANT: LIMITATIONS & RESTRICTIONS" (fontSize 13, fontWeight 700, color "#92400e" dark amber, textTransform uppercase, marginBottom 8)
  - A subheading: "The following limitations apply to this inspection and must be considered when relying on this report." (fontSize 10, color "#92400e", fontStyle italic, marginBottom 12)
  - If `limitations` is present, render the limitations text (fontSize 11, lineHeight 1.7, color "#78350f" dark brown)
  - If limitations is null, render: "No specific limitations were encountered during this inspection. Standard limitations apply as outlined in the scope of works." (same styling)

Below the warning box (with marginTop 16), add a formal court disclaimer in a separate light gray box (backgroundColor "#f9fafb", padding 12, borderWidth 1, borderColor "#e5e7eb"):
- Bold label: "Court & Legal Notice" (fontSize 10, fontWeight 700, marginBottom 6)
- Text: "Areas not inspected or assessed should not be assumed to be free of defects. This report is limited to the scope defined herein. Any party relying on this report must consider the stated limitations. The inspector's opinions are based solely on conditions observable at the time of inspection within the areas accessed." (fontSize 9, lineHeight 1.6, color colors.gray700)

**Styling:**
- Use `styles.page` (with paddingTop: 80, paddingBottom: 60 like declaration.tsx) for both Pages
...
- Limitations page: amber warning box as described above
...
3. Confirm the component renders TWO `<Page>` elements (one for subsections 3.1-3.4, one dedicated to 3.5 Limitations) by searching for `<Page` in the new file
  </verify>
  <done>
- MethodologySection component exists at src/lib/pdf/sections/methodology.tsx
- Component renders Section 3 with subsections 3.1-3.4 on page 1
- Section 3.5 Limitations renders on a dedicated second page with page-break-before
- Limitations page has a full-width amber warning box with bold uppercase heading, italic subheading, and prominent styling
- Limitations page includes a formal court/legal notice disclaimer below the warning box
...
**Step 3: Update the inline TOC to be accurate**
...
**Step 4: Verify compliance section (Section 7) is working**
...
5. Search for "3.5 Limitations" in the inline TOC to confirm the new subsection is listed
6. Search for "BUILDING CODE COMPLIANCE ASSESSMENT" to confirm Section 7 still renders (no regression)
  </verify>
  <done>
...
- Section 3.5 renders on its own dedicated page with prominent amber warning box and court disclaimer
- Inline TOC subsection labels match the actual section content (including 3.5)
...
3. PDF output structure verification:
   ...
   - Section 3.5 renders on its own page with prominent amber warning box and court disclaimer
   - Section 7 renders compliance tables with E2/B2/COP and pass/fail/NA
4. Inline TOC entries match actual section content (including 3.5 Limitations)
...
PDF-03 SATISFIED: PDF includes limitations section (Section 3.5) on dedicated page with amber warning box, bold heading, and court disclaimer — visually prominent and unmissable for court admissibility
